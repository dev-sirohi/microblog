...
internal sealed class InMemoryUtils
{
    ...
    internal static string GetKey(AppConstants.InMemoryOperationType operationType, object? entity1 = null,
        object? entity2 = null)
    {
    ...
    {
        if (value.GetType() == typeof(string) || value.GetType() == typeof(long) || value.GetType() == typeof(double) ||
            value.GetType() == typeof(int) || value.GetType() == typeof(bool) ||
            value.GetType() == typeof(float)) return (RedisValue)value;
    ...
    internal static RedisChannel ConvertToInMemoryChannel(object value)
    {
        return new RedisChannel(
            Convert.ToString(value) ?? throw new Exception("Invalid value conversion to Redis Channel"),
            RedisChannel.PatternMode.Auto);
    }

    internal async Task<List<T>> GetSetMembersAsync<T>(string key)
    {
        var valueList = await _inMemoryDb.SetMembersAsync(key);
    ...
    internal static List<T> GetMembersFromValueListAs<T>(RedisValue[] valueList)
    {
        ...
        return CommonUtils.TransformTo<List<T>>(valueList) ??
               throw new AppException("Invalid conversion of value list");
    }

    internal static double GetUniqueRank(DateTime? timeEntity = null)
    {
        var _timeEntity = DateTime.UtcNow;
        if (timeEntity != null) _timeEntity = timeEntity.Value;
    ...
    internal async Task FlushAndClearQueueOverflow(AppConstants.InMemoryOperationType operationType)
    {
        await _inMemoryDb.Multiplexer
            .GetSubscriber()
            .PublishAsync(ConvertToInMemoryChannel(AppConstants.InMemoryPubSubChannels.FlushAndClearQueueOverflow),
                ConvertToInMemoryValue(operationType));
    }

    internal async Task ClearQueueOverflow(AppConstants.InMemoryOperationType operationType)
    {
        string key = string.Empty;
        var cacheConfig = new AppConstants.CacheConfig();

        switch (operationType)
        {
            case AppConstants.InMemoryOperationType.LIKE_EVENT_FOR_BACKGROUND_SYNC_QUEUE_ADD:
                key = GetKey(operationType);
